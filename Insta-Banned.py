...
def Rep_Insta(total):
	...
	try:		
		if '"authenticated":true' in req1.text:
			...
			print('Worring Password ❌')		
			exit()	
		elif ('"user":false') in req1.text:
			print(' Cant founed the user name  ❌') 
			exit()
	except:
		pass
	...
	while True:
		try:
			...
			if req.json()['status']=="ok":
				print(f"\r[{Fore.MAGENTA}${Fore.RESET}] {Fore.CYAN} The Total nummber of communication {Fore.RESET} [{Fore.GREEN}{total}{Fore.RESET}]",end='')
				total+=1
			else:
				print(f'\n [!] Error Report')
